    ...
    @hash = Gmaps4rails.build_markers(@wants) do |want, marker|
      if user_signed_in?
        if current_user.id == want.USER_ID
          ...
          info = "<div class=\"infowindow\"><h2>" + want.TITLE + " が欲しい！</h2><h3>user id: "  + view_context.link_to( want.USER_ID.to_s, :controller => "contacts", :action => "index" ) + "</h3><p>" + want.COMMENT + "</p></div>";
        end
      end
