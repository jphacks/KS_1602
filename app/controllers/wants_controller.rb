    ...
    @hash = Gmaps4rails.build_markers(@wants) do |want, marker|
      if user_signed_in?
        if current_user.id == want.user_id
          ...
          info = "<div class=\"infowindow\"><h2>" + want.title + " が欲しい！</h2><h3>"  + view_context.link_to("user id: " + want.user_id.to_s , :controller => "users", :action => "index", :id => want.user_id ) + "</h3><p>" + want.comment + "</p></div>";
        end
      end
